...
1. 해당 층의 **바닥 진동센서** 값이 그 층의 충격소음 기준(주/야간)을 넘으면 "이 층 자신"이
   충격소음 발생지로 1차 판정
...
### 1-4. 법정 기준값 (기본값, 층별로 대시보드에서 조정 가능)

공동주택 층간소음의 범위와 기준에 관한 규칙(환경부·국토부) 기준:
...
> 실제 법 규정에는 "1시간 내 3회 이상 초과 시 인정", 최고소음도(Lmax) 기준 등 추가 조건이
> 있습니다. 이 프로젝트는 학습·시연 목적의 근사 구현이며, 실제 민원/제재 근거로 쓰려면
> 정식 소음측정기와 인증된 절차를 따라야 합니다.
...
2. SQL Editor에서 `supabase/migrations/0001_init.sql` 전체 내용 실행
   - `floors`, `sensor_readings`, `noise_events`, `alerts` 테이블 생성
   - 소음 위치 판정 트리거 생성
   - RLS 정책 생성, Realtime 발행 등록
   - 기본 5개 층 시드 데이터 삽입 (필요 시 층 수 조정: `floors` 테이블에 행 추가/삭제)
...
- `/floors/[id]` : 층별 최근 센서 데이터, 경고 이력, 수동 경고 발령 버튼, 층별 소음 기준(dB) 설정
